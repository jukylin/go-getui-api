...
type QueryPushResultByGroupNameResult struct {
...
func GetPushResultByGroupName(appId string, auth_token string, group_name string) (*QueryPushResultByGroupNameResult, error) {
	...
	queryPushResultByGroupNameResult := new(QueryPushResultByGroupNameResult)
	if err := json.Unmarshal([]byte(result), &queryPushResultByGroupNameResult); err != nil {
		return nil, err
	}

	return queryPushResultByGroupNameResult, err
}
